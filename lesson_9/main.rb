...
class Interface
  ...
  def manage_wagon
    ...
    loop do
      ...
      puts '4. Рассадить пассажиров/Загрузить вагоны'
  ...
  def create_wagon
    ...
    if @type == :cargo
      ...
      puts "Создан вагон #{@type} объем которого #{volume} кубов"
    else
      print 'Введите количество мест : '
      seats = gets.chomp.to_i
      @wagons << (PassengerWagon.new seats)
      puts "Создан вагон #{@type} с посадочными местами #{seats} человек"
    end
  end
  ...
  def unhook_wagon
    ...
    @trains.each do |train|
      next unless train.all_wagons.include?(wagon)
  ...
  def busy_wagon
    puts ' Рассадить пассажиров/Загрузить вагоны'
    list wagons
    print 'Выберите Рассадить пассажиров/ загрузить вагоны: '
    wagon = gets.chomp.to_i
    wagon = @wagons[wagon - 1]
    puts "Выбран вагон #{wagon}"
    if @type == cargo
  ...
  def send_train
    ...
    loop do
      ...
      when 1
        train.move_forward
        puts "Предыдущае станция - #{train.previous_station.name}" unless train.previous_station.nil?
        puts "Текущая станция - #{train.current_station.name}"
  ...
  def list_wagons
    @wagons.each.with_index do |_train, index|
  ...
  def seed
    ...
    @trains << (train4 = PassengerTrain.new '444-dd')
    @stations << (station_1 = Station.new 'first1')
    @stations << (station_2 = Station.new 'second')
    @stations << (station_3 = Station.new 'middle')
    @stations << (station_4 = Station.new 'center')
    @stations << (station_5 = Station.new 'five55')
    ...
    @wagons << (wagon_1 = CargoWagon.new(200))
    @wagons << (wagon_2 = PassengerWagon.new(1))
  end
...
run_programm = Interface.new.run
